...
NB: `is.vector()` does not test if an object is a vector. Instead it returns `TRUE` only if the object is a vector with no attributes apart from names. Use `is.atomic(x) || is.list(x)` to test if an object is actually a vector. However, be careful of the context where you want to distinguish vectors because when `x` is a matrix, `is.atomic(x)` will also return true.
